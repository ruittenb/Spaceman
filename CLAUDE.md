...
- **Never show numbers; names in full on one line** — asked 2026-09-17 after the 12pt single row kept auto-shrinking to numbers. Session believes fixed in v1.27.0: auto-shrink (`ShrinkLevel`, `ShrinkOverrides`, the `autoShrink` toggle) removed; replaced by fit-to-width (`fittedSize` in AppDelegate, `IconSize.nextSmaller(twoRows:)`) that only steps the icon size down, keeps it across space switches, and retries the user's size on refresh/topology/unlock. macOS still hides a status item that does not fit at the smallest size; that is the only remaining failure mode. Check: names never replaced by numbers; switching spaces does not blink. v1.28.0 then made the fit MEASURED rather than reactive: budget = status item window `frame.maxX` - `NSScreen.auxiliaryTopRightArea.minX` (956pt on his 16"), largest size whose icon width fits is rendered, occlusion only corrects a too-generous budget. Undecorated spaces now use half horizontal padding (saves 24pt at 12pt). Diagnostics: `log stream --predicate 'subsystem == "io.github.connerstobie.Spaceman" AND category == "fit"'`. **Awaiting Conner's sign-off.**
